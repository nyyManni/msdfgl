...
msdfgl_context_t msdfgl_create_context(const char *version) {
  msdfgl_context_t ctx =
      (msdfgl_context_t)calloc(1, sizeof(struct _msdfgl_context));
  ...
  ctx->_atlas_projection_uniform =
      glGetUniformLocation(ctx->gen_shader, "projection");
  ctx->_texture_offset_uniform =
      glGetUniformLocation(ctx->gen_shader, "offset");
  ...
  ctx->_glyph_height_uniform =
      glGetUniformLocation(ctx->gen_shader, "glyph_height");

  ctx->_meta_offset_uniform =
      glGetUniformLocation(ctx->gen_shader, "meta_offset");
  ctx->_point_offset_uniform =
      glGetUniformLocation(ctx->gen_shader, "point_offset");
  ...
  ctx->_units_per_em_uniform =
      glGetUniformLocation(ctx->render_shader, "units_per_em");
...
msdfgl_atlas_t msdfgl_create_atlas(msdfgl_context_t ctx, int texture_width,
                                   int padding) {
...
int _msdfgl_generate_glyphs_internal(msdfgl_font_t font, int32_t start,
                                     int32_t end, unsigned int range,
                                     int32_t *keys, int nkeys) {
  ...
  for (size_t i = 0; (int)i < (int)nrender; ++i) {
    ...
    msdfgl_glyph_buffer_size(font->face, index, &meta_sizes[i],
                             &point_sizes[i]);
  ...
  for (size_t i = 0; (int)i < (int)nrender; ++i) {
    ...
    buffer_width =
        font->face->glyph->metrics.width / SERIALIZER_SCALE + font->range;
    buffer_height =
  ...
  _msdfgl_ortho(0, (GLfloat)atlas->texture_width, 0,
                (GLfloat)atlas->texture_height, -1.0, 1.0,
                framebuffer_projection);
  _msdfgl_ortho(-(GLfloat)atlas->texture_width, (GLfloat)atlas->texture_width,
                -(GLfloat)atlas->texture_height, (GLfloat)atlas->texture_height,
                -1.0, 1.0, atlas->projection);
...
int msdfgl_generate_glyph(msdfgl_font_t font, int32_t character, void *_user) {
  return _msdfgl_generate_glyphs_internal(font, character, character + 1, 1,
                                          NULL, 0);
}
...
void msdfgl_render(msdfgl_font_t font, msdfgl_glyph_t *glyphs, int n,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (glyphs[i].key >= font->_direct_lookup_upper_limit) {
      msdfgl_map_item_t *e =
          msdfgl_map_get(&font->character_index, glyphs[i].key);
  ...
  glUniformMatrix4fv(font->context->window_projection_uniform, 1, GL_FALSE,
                     projection);
  glUniform1f(font->context->_padding_uniform,
              (GLfloat)(font->range / 2.0 * SERIALIZER_SCALE));
  glUniform1f(font->context->_units_per_em_uniform,
              (GLfloat)font->face->units_per_EM);
...
msdfgl_map_item_t *msdfgl_map_get_or_add(msdfgl_font_t font, int32_t key) {
  ...
  if (!e) {
    if (font->context->missing_glyph_cb) {
      if (!font->context->missing_glyph_cb(
              font, key, font->context->missing_glyph_user_data))
        return NULL;
...
void msdfgl_geometry(float *x, float *y, msdfgl_font_t font, float size,
  ...
  if (flags & MSDFGL_WCHAR) {
    fprintf(stderr,
            "msdfgl: MSDfGL_WHCAR is deprecated, use MSDFGL_UTF8 instead\n");
  ...
  void *s = calloc(bufsize + 1,
                   flags & MSDFGL_WCHAR ? sizeof(wchar_t) : sizeof(char));
  if (!s)
    return;
  va_start(argp, fmt);
  if (flags & MSDFGL_WCHAR)
  ...
  for (size_t i = 0; buf_idx < bufsize; ++i) {
    ...
    if (flags & MSDFGL_KERNING && (prev_key != -1) &&
        FT_HAS_KERNING(font->face))
      FT_Get_Kerning(font->face, FT_Get_Char_Index(font->face, prev_key),
                     FT_Get_Char_Index(font->face, key), FT_KERNING_UNSCALED,
                     &kerning);

    if (flags & MSDFGL_VERTICAL)
      *y += (e->advance[1] + kerning.y) *
            (size * font->context->dpi[1] / 72.0f) / font->face->units_per_EM;
    else
      *x += (e->advance[0] + kerning.x) *
            (size * font->context->dpi[0] / 72.0f) / font->face->units_per_EM;
    prev_key = key;
  }
  free(s);
}

float msdfgl_printf(float x, float y, msdfgl_font_t font, float size,
  ...
  for (size_t i = 0; buf_idx < bufsize; ++i) {
    ...
    if (flags & MSDFGL_KERNING && i && FT_HAS_KERNING(font->face)) {
      ...
      if (flags & MSDFGL_VERTICAL)
        glyphs[i - 1].y += kerning.y * (size * font->context->dpi[1] / 72.0f) /
                           font->face->units_per_EM;
      else
        glyphs[i - 1].x += kerning.x * (size * font->context->dpi[0] / 72.0f) /
                           font->face->units_per_EM;
    }
...
GLuint _msdfgl_atlas_texture(msdfgl_font_t font) {
  return font->atlas->atlas_texture;
}
GLuint _msdfgl_index_texture(msdfgl_font_t font) {
  return font->atlas->index_texture;
}

void msdfgl_set_dpi(msdfgl_context_t context, float horizontal,
                    float vertical) {
